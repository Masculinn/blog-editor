...
export function MediaLibrary({ render, title }: ToolComponentProps) {
  ...
  async function uploadFiles(files: File[]) {
    ...
    setUploadingCount((current) => Math.max(0, current - acceptedFiles.length));
  }
      ...
      {({ close: _ }) => (
        <div className="flex max-h-[80vh] min-h-152 flex-col overflow-hidden">
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 gap-6 lg:grid-cols-[minmax(18rem,0.75fr)_minmax(0,1.75fr)] ">
              ...
              >
                <div className="flex max-w-xs flex-col items-center">
                  ...
                  <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
                    JPEG, PNG, and WebP files are converted to WebP at quality
                    80 before upload.
                  </p>
                </div>
              </div>
            ...
            <div className="min-h-0 overflow-y-scroll scrollbar-custom scroll-fade pr-1">
              {isLoading ? (
                <MediaSkeleton />
              ) : media.length === 0 ? (
                ...
                <div className="grid auto-rows-40 grid-cols-2 gap-3 xl:grid-cols-3">
                  {media.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        {/** biome-ignore lint/performance/noImgElement: dynamic */}
                        <img
                          src={item.publicUrl}
                          alt={item.name}
                          loading="lazy"
                        ...
                        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 bg-linear-to-t from-black/80 via-black/35 to-transparent px-3 pb-3 pt-12">
                        ...
                        <div className="absolute right-2 top-2 z-20 flex translate-y-1 items-center gap-1 opacity-0 transition group-hover:translate-y-0 group-hover:opacity-100 focus-within:translate-y-0 focus-within:opacity-100">
                          <CopyCode
                            data={item.publicUrl}
                            variant="ghost"
