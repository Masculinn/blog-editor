...
const useAnimationMixer = ({
  ...
  useMemo(() => {
    if (!Array.isArray(a)) {
      return reverse
        ? { animate: a.initial, initial: a.animate }
        : { animate: a.animate, initial: a.initial };
    }
    ...
    return reverse
      ? { animate: mergedInitial, initial: mergedAnimate }
      : { animate: mergedAnimate, initial: mergedInitial };
  }, [a, reverse]);
...
const useAnimation = ({
  ...
  useEffect(() => {
    ...
    if (stopAnimation) {
      ...
      dispatch({ payload: { reverseAnimation }, type: "UPDATE" });
    }
